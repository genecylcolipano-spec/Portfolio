...
const SITE_URL = "https://genecyl.com";
const AUTHOR = "Genecyl";
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const findInCache = () => {
      try {
        ...
        return cached.find((p) => toSlug(p.Title) === slug);
      } catch {
        return undefined;
      }
    };
    ...
    const load = async () => {
      // Paint instantly from the cache the portfolio grid already populated
      const cached = findInCache();
      if (cached && !cancelled) {
        setProject(normalizeProject(cached));
        setStatus("ready");
      }

      // Then confirm against Supabase so a direct/shared link also resolves
      ...
      <div className="min-h-screen bg-[#030014] flex items-center justify-center px-4">
        <div className="text-center space-y-6 animate-fadeIn">
          <h2 className="text-2xl md:text-4xl font-bold text-white">
            {status === "error" ? "Something went wrong" : "Project not found"}
          </h2>
          <p className="text-gray-400 max-w-md mx-auto">
            {status === "error"
              ? "We couldn't reach the database. Please try again in a moment."
              : "This project may have been renamed, unpublished, or removed."}
          </p>
          <Link
            to="/"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white font-medium transition-transform duration-300 hover:scale-105"
          >
  ...
  const projectUrl = `${SITE_URL}/project/${toSlug(project.Title)}`;
  const metaDescription = project.Description
    ? project.Description.slice(0, 155)
    : `Project ${project.Title} by ${AUTHOR} — Frontend Web Developer.`;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:title" content={`${project.Title} — ${AUTHOR}`} />
